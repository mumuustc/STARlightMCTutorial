...
then
    if [ $runLocally == true ]; then 
        echo "For local running, Usage: `basename $0` Production_type Channel Beam1_Z Beam2_Z Beam1_A Beam2_A Beam1_Gamma Beam2_Gamma BreakupMode EtaCut EtaMin EtaMax PtCut PtMin PtMax Nevents RandomSeed "
    else echo "For gridpack production, Usage: `basename $0` Production_type Channel Beam1_Z Beam2_Z Beam1_A Beam2_A Beam1_Gamma Beam2_Gamma BreakupMode EtaCut EtaMin EtaMax PtCut PtMin PtMax"
    fi
...
# the detailed parameter settings can be found in Comput. Phys. Commun. 212 (2017) 258
...
## Y(1S) -> mumu
## note, if produce vector meson the W_MAX and W_MIN shoud set to be -1
#prodType=${1:-2}
#channel=${2:-553013}
...
# # retrieve the latest Starlight version
...
W_MAX = 200   #Max value of w
W_MIN = 6    #Min value of w
...
if [ $runLocally == true ]; then
    ...
    if [ $prodType -ge 4 ]; then
        ./starlight < my.input &> log_${prodType}_${BreakupMode}_${channel}_${seed}.txt
    else
        ./starlight &> log_${prodType}_${BreakupMode}_${channel}_${seed}.txt
    fi
    ...
    Beam1Energy=$(echo "${Beam1Gamma} * ${protonMass}" | bc)
    Beam2Energy=$(echo "${Beam2Gamma} * ${protonMass}" | bc)

    #now convert the starlight file to a LHE file
    root -l -b << EOF
    .x convert_SL2LHE.C+(1,"slight.out","slight_${prodType}_${BreakupMode}_${channel}_${seed}",${Beam1Energy},${Beam2Energy})
    .q
EOF
    echo "*** LHE CONVERSION COMPLETE ***"

    echo "Output ready with log_${prodType}_${BreakupMode}_${channel}_${seed}.txt and slight_${prodType}_${BreakupMode}_${channel}_${seed}.lhe at `pwd` and $WORKDIR"
else
    tar -czf slightGridpack_ProdType${prodType}_BreakupMode${BreakupMode}_Channel${channel}_beam${Beam1A}.tgz starlightTrunk convert_SL2LHE.C dpmjetV305
    echo "Created starlight tarball: slightGridpack_ProdType${prodType}_BreakupMode${BreakupMode}_Channel${channel}_beam${Beam1A}.tgz"
fi
